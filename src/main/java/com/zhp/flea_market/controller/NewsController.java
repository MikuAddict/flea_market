...
public class NewsController {
     ...
     */
    @GetMapping("/latest")
    @Operation(summary = "获取最新新闻", description = "获取系统中最新的新闻信息")
    public BaseResponse<List<News>> getLatestNews() {
    ...
    public BaseResponse<Boolean> updateNews(@RequestBody NewsUpdateRequest newsUpdateRequest) {
        ...
        // 检查新闻是否存在
        News existNews = newsService.getById(newsUpdateRequest.getId());
        if (existNews == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "新闻不存在");
        }
    ...
    public BaseResponse<Boolean> deleteNews(
        ...
        // 检查新闻是否存在
        News existNews = newsService.getById(id);
        if (existNews == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "新闻不存在");
        }
